...
if [ "$MODE" = "worker" ]; then
    ...
    exec python -m celery -A core.celery flower --broker="$CELERY_BROKER_URL" --result-backend="$CELERY_RESULT_BACKEND" --port=$PORT --address=0.0.0.0
elif [ "$MODE" = "api" ]; then
    ...
    exec python -m celery -A core.celery flower --broker="$CELERY_BROKER_URL" --result-backend="$CELERY_RESULT_BACKEND" --port=7860 --address=0.0.0.0
else
